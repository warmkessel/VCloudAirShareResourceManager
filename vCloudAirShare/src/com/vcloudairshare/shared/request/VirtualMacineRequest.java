...
public interface VirtualMacineRequest extends RequestContext {
	...
	Request<List<VirtualMachineDTO>> findByAvialable(int from, int count, MachineType machineType, Status status);
	Request<VirtualMachineDTO> findFirstByAvialable(int from, int count, MachineType machineType, Status status);
}